# Dev Agent Lens: installing it

For the person installing Dev Agent Lens on one Linux machine. Every step is a
command and what you should see. Every time given is from a real install on a fresh
Ubuntu 24.04 machine with 4 CPUs and 16 GB of RAM.
...
`dal-litellm-1`. Takes about 30 seconds.
...
About a minute and a half from the start of step 1 to here.
...
`received` is now greater than zero (one short prompt produces about 8). That is
capture working. The prompt itself answers as fast as before; the spans are in the
...
layout; without it you get the raw columns). Three more to try:

```
SELECT date_trunc('day', start_time) AS day, count(*) AS calls FROM spans GROUP BY 1 ORDER BY 1
SELECT tool_name, count(*) FROM spans WHERE tool_name IS NOT NULL GROUP BY 1 ORDER BY 2 DESC LIMIT 10
SELECT session_id, count(*) AS calls, sum(tokens_prompt + tokens_completion) AS tokens FROM spans GROUP BY 1 ORDER BY 3 DESC LIMIT 10
```
...
Three weeks of one person's sessions (about 24,000 spans) loads in a few seconds.
